const ChatPanel = {
  ...
  methods: {
    ...
    async onSessionSelected(session) {
      try {
        ...
        if (result.success) {
          ...
          this.reconnectToStreamingMessages();
        }
      } catch (error) {
    ...
    reconnectToStreamingMessages() {
      // Page-reload resume: any assistant message left at status=
      // 'streaming' is still being generated by a worker thread on
      // the server. Open a follow stream for each so the user sees
      // the rest of the response without having to ask again.
      this.messages.forEach((msg, index) => {
        if (msg && msg.role === "assistant" && msg.status === "streaming") {
          this.followAssistantMessage(index, msg.uuid);
        }
      });
    },

    async followAssistantMessage(assistantIndex, messageUuid) {
      if (!messageUuid) return;
      // Mark the row as live so the existing template branches that
      // gate on msg.streaming (loading dots, "running …" labels)
      // light up the same as during a fresh send.
      this.messages[assistantIndex].streaming = true;
      this.loading = true;

      try {
        for await (const event of window.apiService.followAIMessage(
          messageUuid
        )) {
          if (event.type === "session") {
            this.adoptNewSessionId(event.session_id);
          } else if (event.type === "text") {
            // The server may instruct us to replace rather than append
            ...
            this.messages[assistantIndex].content =
              this.messages[assistantIndex].content ||
              `Error: ${event.error || "Stream interrupted."}`;
          }
        }
      } catch (err) {
        console.error("follow stream failed:", err);
        this.messages[assistantIndex].streaming = false;
      } finally {
        this.loading = false;
      }
